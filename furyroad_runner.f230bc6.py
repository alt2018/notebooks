    ...
    for corpus in hps.corpii:
        ...
        if hasattr( corpus, 'C_transformer' ):
            corpus.C_transformer = build_transformer( corpus.C_transformer, num_features=num_features,
                                                      lookahead=lookahead )
    hps.transformer = build_transformer( hps.transformer, num_features=num_features, lookahead=lookahead )
...
def build_transformer( hps, num_features, lookahead ):
    if hps is None:
        return
    transforms = []
    ...
    if getattr( hps, 'rectifier', False ):
        transforms.append( mx.online.OnlineRectifier( lookahead=lookahead, **hps.rectifier.__dict__ ) )
