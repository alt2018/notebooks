...
def build_colossus( hps, lookahead, num_features, targets, weight, ortho_features, conditioners, dtype, device,
                    verbose ):
    ''' build colossus'''
    hps.alpha.transformer = build_transformer( hps.alpha.transformer, num_features=num_features )
    if hps.omega:
        hps.omega.transformer = build_transformer( hps.omega.transformer, num_features=num_features )
    if hps.gamma:
        hps.gamma.transformer = build_transformer( hps.gamma.transformer, num_features=num_features )
    hps.zeta_transformer = build_transformer( hps.zeta_transformer, num_features=num_features )
    return mxf.Colossus.operator(
        targets=targets, weight=weight, ortho_features=ortho_features, conditioners=conditioners, burn_in=hps.burn_in,
        batch_size=hps.batch_size, minibatch=hps.minibatch, dtype=dtype, device=device, verbose=verbose,
        lookahead=lookahead, alpha=hps.alpha, omega=hps.omega, gamma=hps.gamma, theta=hps.theta,
        zeta_transformer=hps.zeta_transformer,
        halflife_polyak=hps.halflife_polyak, kappa=hps.kappa, halflife_kappa=hps.halflife_kappa, boost=hps.boost,
        tv=hps.tv, ortho=hps.ortho, priming=hps.priming, loss=hps.loss, epsilon=1e-15, name='colossus',
        hyper_tune=hps.hyper_tune, halflifes_adam=hps.halflifes_adam, end_to_end=hps.end_to_end,
    )
